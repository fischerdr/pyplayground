...
# --- API Client ---
# Removed local PXBackupClient definition (now imported from utils.px_api)


# --- Helper Functions ---

# Removed local generate_token definition (now imported from utils.px_api)
...
def filter_clusters(
    ...
    # Regex is only needed if name_pattern is provided
    ...
    for cluster in clusters:
        ...
        if uid and cluster_uid == uid:
            if match_info not in matched:  # Avoid duplicates if both name and uid match
                matched.append(match_info)
                logger.debug(f"Matched cluster by UID: {cluster_name} ({cluster_uid})")
            continue  # UID match takes precedence if both filters are somehow used
        ...
        if name_pattern:
            try:
                ...
                # Potentially exit or just skip name filtering? Skipping for now.
                name_pattern = None  # Stop trying to use the bad pattern
...
def main(
    ...
    try:
        ...
        # --- Main Logic ---
        # Initialize client using imported class
        ...
        if operation == "list":
            ...
            # 2. Filter clusters
            matched_clusters = filter_clusters(all_clusters_raw, cluster_name, cluster_uid)

            if not matched_clusters:
                console.print(
                    f"[yellow]No clusters found matching the criteria (Name: '{cluster_name}', UID: '{cluster_uid}').[/yellow]"
                )
                # No need to exit here, just inform the user
                return  # Exit cleanly if no clusters match
            ...
            for cluster_info in matched_clusters:
                try:
                    ...
                    # Logged in the function, continue to try other clusters
                    console.print(
