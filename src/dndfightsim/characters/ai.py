...
class CombatAI:
    ...
    def calculate_action_weights(
        ...
        """
        weights: List[float] = [1.0] * len(Action)
        ...
        if self.personality == Personality.AGGRESSIVE:
            weights[attack_idx] *= 2.0 if attack_possible else 0.1  # Be aggressive only if possible
        elif self.personality == Personality.CAUTIOUS:
            weights[dodge_idx] *= 1.5
            weights[parry_idx] *= 1.5
            weights[attack_idx] *= 0.7  # Less likely to attack generally
        ...
        if attack_possible:
            ...
            weights[move_idx] *= 2.0  # Encourage moving if cannot attack
