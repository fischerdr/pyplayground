...
def inspect_cluster(client: PXBackupClient, org_id: str, cluster_name: str, cluster_uid: str) -> Dict[str, Any]:
    """
...
# API/Auth Options
@click.option('--api-url', required=True, envvar='PX_BACKUP_API_URL', help='PX-Backup API URL (e.g., px-backup.example.com). Env: PX_BACKUP_API_URL')
@click.option('--org-id', required=True, envvar='PX_BACKUP_ORG_ID', help='Organization ID. Env: PX_BACKUP_ORG_ID')
@click.option('--token', required=False, envvar='PX_BACKUP_TOKEN', help='Authentication token (recommended). Env: PX_BACKUP_TOKEN')
...
@click.option('--auth-url', required=False, envvar='PX_AUTH_URL', help='Authentication server URL (if generating token). Env: PX_AUTH_URL')
@click.option('--client-id', default='px-backup', show_default=True, envvar='PX_CLIENT_ID', help='Client ID for authentication (if generating token). Env: PX_CLIENT_ID')
@click.option('--username', required=False, envvar='PX_USERNAME', help='Username for authentication (if generating token). Env: PX_USERNAME')
@click.option('--password', required=False, envvar='PX_PASSWORD', help='Password for authentication (if generating token). Env: PX_PASSWORD', hide_input=True)
# Operation Selection
@click.option('--operation', type=click.Choice(['list', 'inspect'], case_sensitive=False), default='list', help='Operation to perform.')
# Inspect Options
@click.option('--cluster-name', required=False, help='Name of the cluster to inspect (required for inspect operation).')
@click.option('--cluster-uid', required=False, help='UID of the cluster to inspect (required for inspect operation).')
# Control Options
@click.option('--debug', is_flag=True, default=False, help='Enable debug logging.')
def main(api_url: str, org_id: str, token: Optional[str], no_validate_certs: bool,
         auth_url: Optional[str], client_id: str, username: Optional[str], password: Optional[str],
         operation: str,
         cluster_name: Optional[str], cluster_uid: Optional[str], debug: bool):
    """
    ...
    try:
        ...
        if operation == 'list':
            ...
            if not cluster_name or not cluster_uid:
                # Use ClickException for error handling
                raise click.ClickException("Error: --cluster-name and --cluster-uid are required for the 'inspect' operation.")

            try:
                cluster_details = inspect_cluster(client, org_id, cluster_name, cluster_uid)
