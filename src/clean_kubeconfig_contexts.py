...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(logs_dir, "clean_kubeconfig_contexts.log"), mode="a"),
    ],
)
...
def get_kube_config_data(config_file: str = config.KUBE_CONFIG_DEFAULT_LOCATION) -> Dict[str, Any]:
    """Get kubeconfig data.
    ...
    try:
        ...
        loader = KubeConfigLoader(config_file=config_file)
...
def display_contexts(
    ...
    try:
        ...
        logger.error(f"Failed to display contexts: {e}")
        sys.exit(1)


def show_current_context(config_file: str = config.KUBE_CONFIG_DEFAULT_LOCATION) -> None:
    ...
    try:
        ...
        panel = Panel(content, title="Current Context", border_style="cyan", padding=(1, 2))
...
def set_kube_context(
    ...
    user: str,
    namespace: str,
    config_file: str = config.KUBE_CONFIG_DEFAULT_LOCATION,
) -> None:
    ...
    try:
        ...
        config_dict["contexts"].append(
            {
                "name": context_name,
                "context": {"cluster": cluster, "user": user, "namespace": namespace},
            }
        )
...
def clean_kubeconfig(
    ...
    if kubeconfig:
        logger.info(f"Using custom kubeconfig file: {kubeconfig}")
        # Validate the custom kubeconfig file
        if not os.path.exists(kubeconfig):
            logger.error(f"Custom kubeconfig file not found: {kubeconfig}")
            sys.exit(1)
        config_file = kubeconfig

    # Handle list contexts option
    if list_contexts:
        display_contexts(verbose, config_file)
        return

    # Handle show current context option
    if show_current_context:
        show_current_context(config_file)
        return
    ...
    try:
        ...
        for context in contexts_to_remove:
            if not dry_run:
                delete_kube_context(context, config_file)
            else:
        ...
        for cluster in unused_clusters:
            if not dry_run:
                delete_unused_cluster(cluster, config_file)
            else:
                logger.info(f"Would remove unused cluster: {cluster}")

        for user in unused_users:
            if not dry_run:
                delete_unused_user(user, config_file)
            else:
