...
if __name__ == '__main__':
    ...
    parser.add_argument("--retain",default=None,help="select the number of backups to retain concurrently. This option determines how long backups and snapshots should be kept before they are deleted.")
    parser.add_argument("--betweenFull",default=None,help="specify the number of incremental backups between two full backups. This applies only to Portworx volumes")
    parser.add_argument("--minutes",default=None,help="fixed intervals defined in minutes")
    parser.add_argument("--timeampm",default=None,help="runs every day at a specified time. Provide the hours and minutes from midnight to define the time")
    parser.add_argument("--day",default=None,help="specified day # of the month. If the day given is longer than the current month, it will roll over to the next month.")
    parser.add_argument("--weekday",default=None,help="specified day")
    reqGroup = parser.add_mutually_exclusive_group(required=True)
    reqGroup.add_argument("--file", help="Path to a file containing schedule policy list *NOTE* format(first line must be): schedName,ownername,type,minutes,timeampm,weekday,day")
    reqGroup.add_argument("--type",choices=['interval','daliy', 'weekly', 'monthly'],dest='schedType',default=None,help="Type of schedule policy to create")
    parser.add_argument('-l','--log', help = "Set logging level",choices = ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'],nargs ='?',dest ="loggingLevel",const="INFO", default='INFO',type=str.upper)
    ...
    if args.file:
        ...
        createSchedPol(pxbkapi_url,pxbktkn.token,args.credname,args.orgID,ownerrec['id'],args.accessID,args.secretKey)
