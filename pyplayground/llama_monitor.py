...
from textual.widgets import Footer, Header, Label, Log, Sparkline, Static
...
def _setup_logging(label: Optional[str] = None) -> logging.Logger:
    log_dir = Path("logs")
...
class GpuMetrics:
    ...
    def best_power_w(self) -> float:
        """Pick most informative power reading available."""
...
class ServerMetrics:
    ...
    @property
    def status_text(self)  -> str:  return "● ONLINE" if self.reachable else "○ OFFLINE"
    @property
    def status_color(self) -> str:  return "bright_green" if self.reachable else "red"
    @property
    def current_tps(self)  -> float: return self.tokens_per_sec_history[-1] if self.tokens_per_sec_history else 0.0
    @property
    def current_prompt_tps(self) -> float: return self.prompt_tps_history[-1] if self.prompt_tps_history else 0.0
    @property
...
class LocalCollector:
    ...
    async def fetch_gpu() -> GpuMetrics:
        ...
        try:
            ...
            if isinstance(raw, dict) and "devices" in raw:
                ...
                log.debug("fetch_gpu: amdgpu_top v%s.%s.%s",
                          ver.get("major","?"), ver.get("minor","?"), ver.get("patch","?"))
                devices = raw["devices"]
            ...
            log.debug("fetch_gpu: SCLK=%.0f MCLK=%.0f FCLK=%.0f Power=%.1fW EdgeTemp=%.1f°C",
                      g.gfxclk_mhz, g.mclk_mhz, g.fclk_mhz, g.avg_power_w, g.edge_temp_c)
            ...
            if raw_pwr  is not None: g.socket_power_w = raw_pwr  / 1000.0
            if raw_tgfx is not None: g.gfx_temp_c     = raw_tgfx / 100.0
            if raw_tsoc is not None: g.soc_temp_c      = raw_tsoc / 100.0
            ...
            log.debug("fetch_gpu: socket_power=%.2fW gfx_temp=%.1f°C soc_temp=%.1f°C",
                      g.socket_power_w, g.gfx_temp_c, g.soc_temp_c)
            ...
            for pid_str, entry in fdinfo_raw.items():
                ...
                try:
                    ...
                    log.debug("fetch_gpu: fdinfo pid=%d name=%s VRAM=%.0f GTT=%.0f",
                              pid, name, vram, gtt)
            ...
            g.available = True
            log.info(
                "fetch_gpu OK  VRAM=%0.f/%0.f MiB  GFX=%.1f%%  "
                "Power=%.1fW  SCLK=%.0f  FCLK=%.0f  MCLK=%.0f  "
                "EdgeTemp=%.1f°C  CPUTctl=%.1f°C",
                g.vram_used_mib, g.vram_total_mib, g.gfx_pct,
                g.best_power_w, g.gfxclk_mhz, g.fclk_mhz, g.mclk_mhz,
                g.best_temp_c, g.cpu_tctl_c,
            )
    ...
    def find_llama_pid(port: int) -> Optional[int]:
        ...
        for p in psutil.process_iter(['pid', 'cmdline', 'name']):
            try:
                ...
                pass
        if candidates:
            log.debug("find_llama_pid: fallback pid=%d (port %d not found in cmdline)",
                      candidates[0].pid, port)
    ...
    def collect_proc(prev: ProcMetrics, port: int, gpu: GpuMetrics) -> ProcMetrics:
        ...
        try:
            ...
            p._prev_io_time = now
            p.available = True
            log.debug("collect_proc: pid=%d cpu=%.1f%% rss=%.1fMiB vram=%.0fMiB threads=%d",
                      pid, p.cpu_pct, p.rss_mib, p.vram_mib, p.num_threads)
    ...
    def collect_sys() -> SysMetrics:
        ...
        try:
            ...
            s.tasks_running = sum(1 for p in procs
            ...
            try:
                s.kthreads = sum(
                    1 for p in psutil.process_iter(['pid', 'ppid', 'name'])
                    if p.info['ppid'] in (0, 2) or p.info['pid'] == 2
                    or (p.info['name'] or '').startswith('[')
                )
            except Exception:
                s.kthreads = 0
            ...
            log.debug("collect_sys: cpu=%.1f%% mem=%.0f/%.0fMiB tasks=%d threads=%d load=%.2f uptime=%s",
                      s.cpu_pct_total, s.mem_used_mib, s.mem_total_mib,
                      s.tasks_total, s.threads_total, s.load_1, s.uptime_str)
        except Exception as exc:
...
async def fetch_llama_metrics(url: str, timeout: float = 5.0) -> tuple:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            # On first successful fetch, dump raw text + key list so we can
            ...
            if url not in _METRICS_DUMP_DONE:
                ...
                try:
                    ...
                    p = Path("logs") / f"llama_metrics_{safe}.txt"
                    p.write_text(r.text)
...
def apply_llama_metrics(s: ServerMetrics, raw: dict, now: float) -> None:
    ...
    s.prompt_tokens_total = _pick(raw,
        "llamacpp:prompt_tokens_total",
        "llama_prompt_tokens_total",
        "prompt_tokens_total",
    )
    s.generated_tokens_total = _pick(raw,
        "llamacpp:tokens_predicted_total",
    ...
    log.debug(
        "apply_llama [%s]: gen_tps_gauge=%.2f prompt_tps_gauge=%.2f "
        "gen_total=%.0f prompt_total=%.0f kv=%.1f%% busy=%.1f proc=%d",
        s.name, gen_tps_gauge, prompt_tps_gauge,
        s.generated_tokens_total, s.prompt_tokens_total,
        s.kv_cache_usage, s.n_busy, s.requests_processing,
    )
...
class ServerCard(Widget):
    ...
    def refresh_data(self, use_fahrenheit: bool = False) -> None:
        ...
        self.query_one(f"#{sid('title')}", Label).update(
            f"[bold]{s.name}[/]  [{s.status_color}]{s.status_text}[/]"
        )
        ...
        if not p.available:
            proc_lines = "[bold green]\u2500\u2500 Process  (psutil) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500[/]" + NL + "[dim red]" + (p.error or "no data yet") + "[/]"
        else:
            rows = [
...
class LlamaMonitor(App):
    ...
    def __init__(self, server_configs: list | None = None):
        super().__init__()
        self.servers: list[ServerMetrics] = [
            ServerMetrics(name=c["name"], url=c["url"].rstrip("/"))
            for c in (server_configs or DEFAULT_SERVERS)
        ]
        self._poll_timer: Optional[Timer] = None
        self.use_fahrenheit: bool = False
        log.info("LlamaMonitor init: %d server(s): %s",
                 len(self.servers), [s.url for s in self.servers])
    ...
    async def poll_all(self):
        ...
        llama_results = await asyncio.gather(
            *[fetch_llama_metrics(s.url) for s in self.servers]
        )
        ...
        proc_results = await asyncio.gather(*[
            loop.run_in_executor(
                None, LocalCollector.collect_proc,
                s.proc, _port_from_url(s.url), gpu_data,
            )
        ...
        status = (
            f" {ts}  │  {online}/{len(self.servers)} online  │  {gpu_ok}"
            + (
        ...
        log_msg = (
            ...
            + (
                ...
                f"  {gpu_data.best_power_w:.1f}W  {_tc(gpu_data.best_temp_c, self.use_fahrenheit)}"
                if gpu_data.available else f"  {gpu_data.error}"
            )
            + f"  CPU {sys_data.cpu_pct_total:.0f}%  RAM {sys_data.mem_pct:.0f}%"
    ...
    def action_add_server(self):
        def on_result(r):
            if r:
                ...
                self.query_one("#cards_container", ScrollableContainer).mount(
                    ServerCard(s, id=f"card_{_safe(s.name)}")
                )
                self.poll_all()
    ...
    def action_set_cols(self, n: str):
        self.query_one("#cards_container", ScrollableContainer)\
            .styles.grid_size_columns = int(n)
...
def main():
    ...
    parser.add_argument("--servers", "-s", nargs="+", metavar="NAME=URL",
                        help="e.g.  case=http://localhost:10000")
    ...
    if args.debug:
        logging.getLogger("llama_monitor").setLevel(logging.DEBUG)
