    ...
    payload = json.dumps(
        {
            "metadata": {
                ...
                "ownership": {
                    "owner": ownerid,
                    "groups": [{"id": "px-admin-group", "access": "Admin"}],
                    "public": {},
                },
            },
            "cloud_credential": {
                "type": "AWS",
                "aws_config": {"access_key": accessID, "secret_key": secretKey},
            },
        }
    )
    pxbkheaders = {
...
if __name__ == "__main__":
    ...
    exclusive_group.add_argument(
        "--s3cred-file",
        help="Path to a file containing AWS cred list format: name:owner_username:accessid:secretID",
    )
    exclusive_group.add_argument(
        "-add",
        default=False,
        action="store_true",
        help="Add single entry with credname,accessID,secretkey,owner-name as REQUIRED parms",
    )
