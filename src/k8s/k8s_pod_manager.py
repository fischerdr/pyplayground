...
def find(ctx, pod_name: str):
    ...
    log_level = logging.DEBUG if ctx.obj.get("VERBOSE", False) else logging.INFO
    setup_logging(level=log_level, script_name="k8s_pod_manager_find")
...
def inspect(ctx, pod_name: str, namespace: str, container_name: Optional[str], tail: int):
    ...
    setup_logging(level=log_level, script_name="k8s_pod_manager_inspect")
    logger = get_logger(__name__)  # Re-get logger after setup
