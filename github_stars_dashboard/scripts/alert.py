...
class AlertRule:
    ...
    def __init__(
        ...
        self.message_template = message_template or (
            f"Alert: {metric_name} is {condition} {threshold}"
        )
    ...
    def generate_message(self, value: float) -> str:
        ...
        try:
            ...
            return (
                f"{self.message_template} (value: {value}, threshold: {self.threshold})"
            )
...
class AlertManager:
    ...
    def _send_to_console(self, alert: Alert) -> bool:
        ...
        Returns:
            True if send successful.
        """
        severity_icon = {"low": "L", "medium": "M", "high": "H", "critical": "!"}.get(
            alert.severity, "?"
        )

        print(
            f"[{alert.datetime_str}] [{severity_icon}] {alert.title}: {alert.message}"
        )
        return True
    ...
    def get_alert_summary(self) -> str:
        ...
        for alert in self.alerts[-10:]:
            ...
            summary.append(
                f"  [{status}] [{alert.severity}] {alert.title}: {alert.message[:50]}"
            )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Manage alerts for GitHub Stars Dashboard"
    )
    parser.add_argument(
        "-c", "--config", default="alert_config.json", help="Alert config file"
    )
