...
def px_exec(pod_name, cmd):
    ...
    full_cmd = (
        f'kubectl exec -it {pod_name} -n {PX_NAMESPACE} -c {px_container} -- {nsenter_cmd} "{cmd}"'
    )
...
def get_cm_cloud_drive():
    cm_cloud_drive = shell_exec(
        False, "kubectl get cm -n kube-system | grep px-cloud-drive"
    ).split()[0]
    ...
    if (
        "annotations" in cm_data["metadata"]
        and "kubectl.kubernetes.io/last-applied-configuration"
        in cm_data["metadata"]["annotations"]["kubectl.kubernetes.io/last-applied-configuration"]
    ):
...
def update_vmdk_path_helper(
    json_file_to_modify, new_datastore_full_name, destination_file, old_vmdk_path, new_vmdk_path
):
    try:
...
def get_disks_to_detach(node_id):
    ...
    if vmdk_count != vdisk_count:
        print(
            f"Mismatch in VMDK count({vmdk_count}) and actual vDisks on the VM ({vdisk_count}). Node ID {node_id}. Exiting. Rerun"
        )
        exit(0)
...
def update_one_node(dry_run, node_id, input_file):
    ...
    ###################------------STEP 2----------------------#####################
    print(
        "----- STEP 2: Move VMDKs belonging to node "
        + node_id
        + " and have the new configmap at "
        + config_map_file
    )
    ...
    cm_cloud_drive = (
        subprocess.check_output(
            "kubectl get cm -n kube-system | grep px-cloud-drive | awk '{print $1}'", shell=True
        )
        .decode()
        .strip()
    )
...
def get_short_name(ds_name):
    if ds_name not in shortNameMap:
        ...
        short_name = (
            subprocess.check_output(
                ["jq", ".datastores[].summary.datastore.value"], input=short_name.encode()
            )
            .decode()
            .strip()
        )
...
def pre_reqs(input_file):
    ...
    filename = f"Primary_backup_{timestamp}.yaml"
    print(
        "\n#-- Taking a second backup of the configmap at {}".format(
    ...
    node_ids = (
        subprocess.check_output("awk -F ',' '{print $1}' " + input_file + " | sort -u", shell=True)
        .decode()
        .strip()
    )
    ...
    for node_id in node_ids.split():
        print(
            f"python3.6 pool_rebalance.py rebalance start --node-id {node_id} --input-file {input_file}"
        )
...
def update_vmdk_paths(
    json_file_to_modify, ds_mapping_file, destination_file, old_vmdk_path, new_vmdk_path
):
    try:
        ...
        if new_ds_name_input not in shortname_dict and new_ds_name_input not in fullname_dict:
            print(
                f"Can't find the datastore {new_ds_name_input} in the input file of datastore mappings"
            )
        ...
        if new_ds_name_input in fullname_dict:
            ...
            new_vmdk_path = new_vmdk_path.replace(
                new_ds_name_input, fullname_dict[new_ds_name_input]
            )

        update_vmdk_path_helper(
            json_file_to_modify,
            new_datastore_full_name,
            destination_file,
            old_vmdk_path,
            new_vmdk_path,
        )
    ...
    print(
        compare_json_strings(
            pretty_format_cd(json_file_to_modify), pretty_format_cd(destination_file)
        )
    )
...
def main():
    ...
    parser_update.add_argument(
        "--input-json-file",
        required=True,
        help="output of `kubectl get cm <px-cloud-drive-cofigmap> -ojson -n kube-system",
    )
    parser_update.add_argument(
        "--ds-mapping-file",
        required=True,
        help="A csv file in which each line looks something like this `datastore-2334, SOME-DATASTORE-NAME`",
    )
    parser_update.add_argument(
        "--old-vmdk-path",
        required=True,
        help="Full path of VMDK to replace. [datastore_name] fcd/xxx/xxxxxxxxxxxxxx.vmdk",
    )
    parser_update.add_argument(
        "--new-vmdk-path", required=True, help="Full path of VMDK to replace with."
    )
    ...
    parser_start = parser_rebalance.add_subparsers(
        title="Rebalance Start", dest="rebalance_subcommand"
    )
    ...
    parser_start_start.add_argument(
        "--input-file",
        required=True,
        help="csv file containing src-dst mapping of VMDKs - vmdk_src_dst_mappikngs.csv",
    )
    ...
    parser_start_pre_req.add_argument(
        "--input-file", required=True, help="Input file for prerequisites"
    )
    ...
    if args.subcommand == "update-vmdk-paths":
        update_vmdk_paths(
            ...
            args.ds_mapping_file,
            args.input_json_file + "-modified.json",
            args.old_vmdk_path,
            args.new_vmdk_path,
        )
