...
def get_resources_for_crd(api_client, crd, namespace):
    ...
    try:
        ...
        )  # Specify the current namespace
        return resources.get("items", [])
    except ApiException as e:
        if e.status != 404:  # Log only non-404 errors
            print(
                f"Error fetching resources for CRD {crd.metadata.name} in namespace {namespace}: {e}"
            )
        return []
...
def find_known_and_unknown_crds(api_client):
    ...
    for crd in crds:
        ...
        for namespace in namespaces:
            ...
            if resources:
                print(
                    f"  - Namespace {namespace.metadata.name}: Known CRD with {len(resources)} instances"
                )
