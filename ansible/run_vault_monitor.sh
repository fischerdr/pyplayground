#!/bin/bash
# Shell script to run Vault multi-namespace monitoring using the Ansible role
# This script provides an easy way to run the role with different configurations
...
Vault Multi-Namespace Monitoring - Ansible Role Runner
...
Optional Options:
  ...
  -e, --environment ENV            Environment to use from inventory
...
Examples:
  ...
  $0 -i inventory_vault_monitor.yml -e production
...
run_playbook() {
    ...
    local cmd="ansible-playbook playbook_vault_monitor.yml"
...
if [[ -n "$SECRET_PATHS" && -n "$VAULT_NAMESPACES" ]]; then
