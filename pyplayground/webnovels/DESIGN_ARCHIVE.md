# Glossary & Reader Redesign -- Archive

Closed, fully-resolved investigation and implementation entries moved out of `DESIGN.md` on 2026-07-31 to keep that doc lean. Nothing here is actionable or open -- for current behavior, read `GLOSSARY_ARCHITECTURE.md`; for anything still open or in progress, read `DESIGN.md` itself. Entries below are verbatim, in their original relative order, unedited except where explicitly noted (one stale header corrected).
...
### 2026-07-27: cross-dialog stale-overwrite bug -- confirmed real via live reproduction, fixed and live-verified (2026-07-28)
