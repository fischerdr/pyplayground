...
annotations (`px/vault-namespace`, `px/secret-name`), and exports details
including PVC name, PV name, Vault annotations, and labels from `pxctl volume inspect`.

Example usage:
    python pyplayground/k8s/k8s_px_pvc_data_exporter.py --kubeconfig ~/.kube/config --output-file /tmp/pvc_data.json
"""
...
# --- Kubernetes & Portworx Helper Functions ---
...
def get_vault_secret_details(
    ...
    try:
        ...
        if response and "data" in response:
            return response["data"]  # This contains 'data' and 'metadata' keys
        else:
            logger.warning(f"Secret found at '{secret_path}' but it contains no data.")
            return {"error": "Secret found but contains no data"}

    except hvac.exceptions.InvalidPath:
        logger.warning(f"Invalid Vault path for secret '{secret_path}'.")
        return {"error": "Secret not found (invalid path)"}
    except hvac.exceptions.Forbidden:
        logger.warning(f"Vault access forbidden for secret path '{secret_path}'.")
        return {"error": "Vault access forbidden"}
    except Exception as e:
...
def _filter_volume_labels(labels: Optional[Dict[str, str]]) -> Dict[str, str]:
    """Filters volume labels for keys starting with specific prefixes."""
    if not labels:
        return {}
...
) -> Optional[tuple[Dict[str, str], str, tuple[str, str], List[str]]]:
...
def gather_pvc_data(
    core_v1: client.CoreV1Api, storage_v1: client.CoreV1Api, px_namespace: str
    ...
    # 3. Process each PVC and collect data for export
    ...
    with console.status("[bold green]Processing PVCs...") as status:
        for pvc in annotated_pvcs:
            ...
            # Get Portworx Volume Labels
            if pv_name:
                pxctl_json, _, err_msg = execute_pxctl_inspect(
                    core_v1,
                    px_namespace,
                    px_pod_name,
                    px_container_name,
